  ...
  async function fetchBalance() {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (!res.ok) {
        ...
        // 401/403 直接判定为凭证失效，不重试、不降级，明确提示用户
        if (res.status === 401 || res.status === 403) {
          return { ok: false, code: 'TOKEN_EXPIRED', error: 'Token 已失效，请重新配置' }
        }
      ...
      if (data.code !== 0) {
        // LongCat 凭证失效时通常返回 200 + 业务错误码，按 code/msg 关键词识别
        const bizCode = Number(data.code)
        const bizMsg = typeof data.msg === 'string' ? data.msg : ''
        if (bizCode === 401 || bizCode === 403 || /未登录|token|credential|auth/i.test(bizMsg)) {
          return { ok: false, code: 'TOKEN_EXPIRED', error: 'Token 已失效，请重新配置' }
        }
        return { ok: false, code: 'SHAPE', error: '接口返回结构异常' }
      }
      if (!data.data || !data.data.currentLot) {
        return { ok: false, code: 'SHAPE', error: '接口返回结构异常' }
      }
  ...
  function getBalance() {
    ...
    balanceInFlight = fetchBalance()
      .then((payload) => {
        ...
        // 确定性错误（Token 缺失/失效/配置错误等）时清空旧缓存，防止后续网络抖动时拿过期缓存做错误降级
        if (!payload.transient) {
          balanceCache = null
          console.error('[dsh-balance-phoebe]', payload.code, payload.error)
          return payload
        }
