(function () {
  ...
  function savePosition() {
    try { localStorage.setItem('dshpw-pos', JSON.stringify({ h: state.h, v: state.v, hOff: state.hOff, vOff: state.vOff })) } catch (err) {}
  }
  ...
  function loadPosition() {
    try {
      ...
      if (state.h === 'right') { state.left = Math.max(0, vp.w - w - state.hOff) }
      else if (state.h === 'left') { state.left = state.hOff }
      else { state.left = clamp(vp.w * 0.5 - w * 0.5, 0, Math.max(0, vp.w - w)) }
      if (state.v === 'bottom') { state.top = Math.max(BUBBLE_RESERVED_SPACE, vp.h - h - state.vOff) }
      else if (state.v === 'top') { state.top = Math.max(BUBBLE_RESERVED_SPACE, state.vOff) }
      else { state.top = clamp(vp.h * 0.5 - h * 0.5, BUBBLE_RESERVED_SPACE, Math.max(0, vp.h - h)) }
      return true
  ...
  function showBalance(data) {
    ...
    if (data && data.ok) {
      ...
      var errMsg = (data && data.error) ? String(data.error) : '未配置 Token'
      if (data && data.code === 'TOKEN_EXPIRED') errMsg = 'Token 已失效，请重新配置'
      showBubble('<div class="dshpw-balance-amount">--</div><div class="dshpw-balance-hint">' + escapeHtml(errMsg.slice(0, 30)) + '</div>')
    }
